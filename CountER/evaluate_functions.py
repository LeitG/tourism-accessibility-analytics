import numpy as np
import torch
from sklearn.metrics import ndcg_score, precision_score, roc_auc_score, recall_score, f1_score

def generate_binary_list(input_list, n):
    sorted_list = sorted(input_list, reverse=True)
    binary_list = [1 if x in sorted_list[:n] else 0 for x in input_list]
    return binary_list
...
def compute_ndcg(test_data, user_feature_matrix, item_feature_matrix, k, model, device):
    ...
    with torch.no_grad():
        for row in test_data:
            ...
            ndcgs.append(ndcg)
            roc.append(roc_auc_score(gt_labels, generate_binary_list(scores, k)))
            pre.append(precision_score(gt_labels, generate_binary_list(scores, k)))
            re.append(recall_score(gt_labels, generate_binary_list(scores, k)))
            f1.append(f1_score(gt_labels, generate_binary_list(scores, k)))
    ave_ndcg = np.mean(ndcgs)
    print("auc_score", np.mean(roc))
    print("precision_score", np.mean(pre))
    print("recall_score", np.mean(re))
