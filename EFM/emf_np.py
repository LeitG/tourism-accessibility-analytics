...
class EMF_NP(BaseEstimator, RegressorMixin):
    ...
    def __init__(self, p=8, r=100, total_r=100, lambda_x=0.05, lambda_y=0.05, lambda_h=0.05, lambda_u=0.05,
                 lambda_v=0.05, T=2, Y=np.zeros(shape=(10, 10))):
        """
    ...
    def min_alg(self):
        ...
        # V -> p * r
        V_den = np.dot(self.V, (self.lambda_x * np.dot(self.U1.T, self.U1) + self.lambda_y * np.dot(self.U2.T,
                                                                                                    self.U2) + self.lambda_v * np.identity(
            self.U1.shape[1])))
        # print(V_den.shape)
        V_den = V_den + 1e-10
        ...
        U1_den = np.dot((np.dot(self.U1, self.U2.T) + np.dot(self.H1, self.H2.T)), self.U2) + np.dot(self.U1, (
                    self.lambda_x * np.dot(V_.T, V_) + self.lambda_u * np.identity(V_.shape[1])))
        self.U1 = U1_ * np.sqrt(U1_num / U1_den)

        U2_num = np.dot(Z.T, U1_) + self.lambda_y * np.dot(self.Y, V_)
        U2_den = np.dot((np.dot(self.U2, U1_.T) + np.dot(self.H2, self.H1.T)), U1_) + np.dot(self.U2, (
                    self.lambda_y * np.dot(V_.T, V_) + self.lambda_u * np.identity(V_.shape[1])))
    ...
    def get_all_rating(self):
        """
        Get the predicted rating of all users and all items
        """
