...
def parse_urqlcache(contents):
    ...
    x = json.loads(urqlcache)['results']
    for k in x.keys():
...
def parse_hotel_page(contents):
    ...
    # information.
    address, url, latitude, longitude = '', '', '', ''
    reviewListPage = ''
    hotelInfo = {}
    for k in urqlcache:
        if not contactLinks:
...
class Author:
    ...
    def __init__(self, profile):
        ...
        if profile.get('hometown', None) is not None and profile['hometown'].get('location', None) is not None:
...
class Review:
    ...
    def __init__(self, review, hotel):
        ...
        self.created = review.get('createdDate', '')
        # self.published = review['publishedDate']
        self.published = review.get('publishedDate', '')
        # self.title = review['title']
        self.title = review.get('title', '')
        # self.text = review['text']
        self.text = review.get('text', '')
        # self.room_tip = review['roomTip']
        self.room_tip = review.get('roomTip', '')
        ...
        self.ratings = {'Overall': review.get('rating', '')}
        for rating in review.get('additionalRatings', ''):
            self.ratings[rating['ratingLabel']] = rating.get('rating', '')

        self._author = Author(review.get('userProfile', ''))
...
# Sydney_New_South_Wales -> 255060
parser.add_argument('-r', '--region', default=[255060], type=list, required=False, metavar='REG',
                    help='scrap hotel from region REG; can be added multiple times')
# parser.add_argument('-r', '--region', default=255055, type=int, required=False, action='append', metavar='REG',
#                     help='scrap hotel from region REG; can be added multiple times')
